...
MultiAutocompleteCtrl.$inject=['$filter','$http','$log'];function multiAutocompleteDirective(){return{restrict:'E',controllerAs:"vm",controller:'MultiAutocompleteCtrl',scope:{placeholder:'=?',modelArr:'=ngModel',apiUrl:'@?',suggestionsArr:'=?',objectProperty:'=?',disable:'=?',multiple:'=?',clearAll:'=?',closeOnSelect:'=?',sortBy:'=?'},bindToController:true,templateUrl:'multi-select-autocomplete.html',link:function(scope,element,attr){scope.isRequired=attr.required;scope.errMsgRequired=attr.errMsgRequired;scope.name=attr.name;}};}
...
if(vm.sortBy&&vm.sortBy!==""){vm.suggestionsArr=$filter('orderBy')(vm.suggestionsArr,vm.sortBy);}
